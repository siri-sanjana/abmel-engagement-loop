...
export const Layout = ({ children }: { children: React.ReactNode }) => {
  ...
  return (
    <div className="min-h-screen bg-slate-950 font-sans text-slate-100 flex">
      ...
      <div
        className={`fixed inset-y-0 left-0 z-50 transform transition-transform duration-300 ease-in-out lg:translate-x-0 ${isMobileNavOpen ? "translate-x-0" : "-translate-x-full"} lg:static lg:block`}
      >
        ...
        <header className="h-16 bg-slate-900 border-b border-slate-800 sticky top-0 z-40 px-4 sm:px-6 flex items-center justify-between shadow-sm">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center text-sm text-slate-400 font-medium hidden sm:flex">
              <span className="hover:text-slate-100 cursor-pointer transition-colors">
                Campaigns
              </span>
              <span className="mx-2 text-slate-700">/</span>
              <span className="text-blue-100 bg-blue-500/10 px-2 py-0.5 rounded text-xs border border-blue-500/20">
                Q1 Product Launch
              </span>
            </div>
          </div>
              ...
              {isProfileOpen && (
                <div className="absolute top-12 right-0 w-48 bg-slate-900 rounded-xl shadow-xl border border-slate-800 py-1 z-50 animate-in fade-in zoom-in-95 duration-200">
                  ...
                  <button
                    ...
                    className="w-full text-left px-4 py-2 text-sm text-slate-300 hover:bg-slate-800 hover:text-indigo-400 transition-colors flex items-center gap-2"
                  >
