...
export const CreativeReview = () => {
  ...
  const handleEditImage = async (variant: any) => {
    ...
    try {
      ...
      await generateImageForCreative({
        ...variant,
        visual_prompt: enrichedPrompt,
        backendPreference: "POLLINATIONS",
      });
  ...
  const variants =
    rawVariants.length > 0
      ? rawVariants.map((v: any) => {
          const variant = { ...v };
          if (!variant.why_this_works)
            variant.why_this_works =
              "Selected for maximum engagement alignment.";
          if (!variant.visual_prompt)
            variant.visual_prompt =
              variant.visualDescription || "No visual description available.";
      ...
      : [
          {
            ...
            platform: "Instagram",
            why_this_works:
              "Failsafe activation: Ensuring delivery of premium creative asset.",
            tone: "Premium",
          },
        ];
  ...
  const renderVisualArea = (variant: any) => {
    ...
    const isLocalLoading =
      imageLoadingStates[variant.id] !== false && !isPlaceholder;
      ...
      <>
        ...
        <div className="absolute inset-0 flex items-center justify-center p-12 pointer-events-none">
          <div className="text-center max-w-md relative z-10">
            ...
            <p className="font-serif italic text-2xl text-white/90 leading-relaxed drop-shadow-md text-shadow line-clamp-3">
              "{variant.visual_prompt}"
            </p>
          </div>
        </div>
      ...
      <div className="flex items-center justify-between border-b border-slate-200 pb-6 bg-white">
        <div>
          <div className="flex items-center gap-2 mb-1">
            <span className="px-2 py-0.5 rounded bg-green-100 text-green-700 text-[10px] font-bold uppercase tracking-wide border border-green-200">
              State: Complete
            </span>
            <span className="px-2 py-0.5 rounded bg-slate-100 text-slate-600 text-[10px] font-bold uppercase tracking-wide border border-slate-200">
              ID: #992-AC
            </span>
          </div>
          ...
          <p className="text-slate-500 mt-1">
            ABMEL has selected and verified the optimal creative strategy.
          </p>
        </div>
        ...
        <div className="lg:col-span-8 space-y-4">
          <div className="flex items-center justify-between mb-2">
            <h2 className="text-lg font-bold text-slate-900 flex items-center gap-2">
              {currentCreative?.is_best_creative ||
              currentCreative?.rank === "BEST" ? (
                <>
            ...
            <div className="flex items-center gap-3">
              ...
              <button
                onClick={handlePrev}
                className="p-2 rounded-full border border-slate-200 bg-white hover:bg-slate-50 text-slate-700 transition-colors shadow-sm"
              >
                <ChevronLeft size={20} />
              </button>
              <button
              ...
              >
                ...
                <div className="absolute top-6 right-6 z-20 flex flex-col gap-2 items-end">
                  ...
                  currentCreative?.rank === "BEST" ? (
                    <div className="bg-green-500 text-white text-xs font-bold px-4 py-1.5 rounded-full uppercase tracking-wider shadow-lg flex items-center gap-1.5">
                      <Zap size={14} className="fill-current" /> Highest
                      Weighted Score
                    </div>
                  ) : (
                  ...
                  <div className="bg-white/90 backdrop-blur text-slate-900 text-xs font-bold px-3 py-1.5 rounded-full shadow-sm border border-slate-200/50">
                    {guardrailsNode?.result?.compliance_report?.overall_score ||
                      99}
                    % Safety Score
                  </div>
                </div>
                ...
                <div className="h-[72%] bg-slate-900 relative overflow-hidden group-hover:bg-slate-800 transition-colors">
                  ...
                  <span className="absolute bottom-6 left-6 z-30 text-white text-[10px] items-center gap-1.5 hidden group-hover/btn:flex bg-slate-900 px-2 py-1 rounded-md animate-in fade-in slide-in-from-top-1">
                    Engine:{" "}
                    {currentCreative.visual_assets?.[
                      selectedAssetIndices[currentCreative.id] || 0
                    ]?.provider ||
                      currentCreative.visual_asset_provider ||
                      "Default AI"}
                  </span>
                  <button
                    onClick={async () => {
                      const url =
                        ...
                        currentCreative.visual_asset_url ||
                      ...
                      if (url) {
                        await SupabaseService.getInstance().downloadImage(
                          url,
                          `creative_${currentCreative.id}_${currentIndex}.png`,
                        );
                      }
                    }}
                  ...
                  <div className="flex gap-2 items-center">
                    <input
                      ...
                      onChange={(e) =>
                        setEditPrompts((prev) => ({
                          ...prev,
                          [currentCreative.id]: e.target.value,
                        }))
                      }
                      onKeyDown={(e) =>
                        e.key === "Enter" && handleEditImage(currentCreative)
                      }
                    ...
                    <button
                      ...
                      disabled={
                        ...
                        !editPrompts[currentCreative.id]?.trim()
                      }
                  ...
                  <div className="mt-8 flex items-center justify-between">
                      ...
                      {(() => {
                        ...
                        if (isPlaceholderOrFallback) {
                          return (
                            <button
                              onClick={() =>
                                generateImageForCreative(currentCreative)
                              }
                    ...
                    <div className="flex items-center gap-4 text-right">
                      ...
                      <div>
                        ...
                        <div className="text-slate-900 font-bold">
                          {currentCreative.strategy ||
                            currentCreative.strategy_type ||
                            "Optimized"}
                        </div>
                      </div>
                    </div>
          ...
          <div className="bg-slate-900 rounded-2xl p-6 text-white shadow-lg space-y-4 transition-all duration-300 relative overflow-hidden">
            ...
            <div className="space-y-1">
              ...
              <div className="bg-white/10 rounded-lg p-3 text-xs text-slate-300 border border-white/5 leading-relaxed">
                {currentCreative.why_this_works ||
                  currentCreative.why_it_works ||
                  [
            ...
            <div className="space-y-1">
              ...
              <p className="text-slate-300 text-sm font-medium">
                {currentCreative.target_persona_trait ||
          ...
          <button
            onClick={() => {
              ...
              const dataStr =
                ...
                encodeURIComponent(JSON.stringify(finalResult, null, 2));
              const downloadAnchorNode = document.createElement("a");
              downloadAnchorNode.setAttribute("href", dataStr);
              downloadAnchorNode.setAttribute(
                "download",
                "ABMEL_CAMPAIGN_EXPORT.json",
              );
    ...
    <div className="flex justify-between items-end mb-2">
      <div>
        ...
        <div className="text-2xl font-bold text-slate-900 leading-none">
          {score}
          <span className="text-sm text-slate-500 ml-0.5">{suffix}</span>
        </div>
      </div>
