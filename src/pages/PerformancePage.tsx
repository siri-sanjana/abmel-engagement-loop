    ...
    <div className="space-y-8 animate-in fade-in duration-500 pb-12">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-400 mt-2">
            Performance metrics across {campaigns.length} campaign
      ...
      <div className="bg-indigo-950/40 border border-indigo-500/20 rounded-xl p-4 flex items-start gap-3">
        ...
        <p className="text-sm text-indigo-200/80 leading-relaxed">
          This page displays performance metrics collected from campaign
          executions. The system aggregates engagement signals such as
          impressions, clicks, conversions, and platform distribution to
          evaluate campaign effectiveness and optimize creative strategies.
        </p>
      </div>
        ...
        {[
          {
            ...
            val: formatNumber(metrics.impressions),
            delta: "+12%",
            icon: Eye,
            color: "blue",
          },
          {
            label: "Est. Clicks",
            val: formatNumber(metrics.clicks),
            delta: "+8.5%",
            icon: MousePointer,
            color: "green",
          },
          {
            label: "Conversions",
            val: formatNumber(metrics.conversions),
            delta: "+2.3%",
            icon: TrendingUp,
            color: "indigo",
          },
          {
            ...
            val: formatNumber(metrics.reach),
            delta: "+5%",
            icon: Users,
            color: "purple",
          },
          ...
          >
            <div className="flex justify-between items-start mb-3">
              <div
                className={`p-2.5 rounded-lg bg-${stat.color}-500/10 text-${stat.color}-500`}
              >
                <stat.icon size={18} />
              </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-slate-900 border border-slate-800 rounded-xl p-6">
          ...
          <p className="text-xs text-slate-500 mb-5">
            Campaign performance score over the last 12 weeks
          </p>
        ...
        {creativeStats.length > 0 ? (
            ...
            <table className="w-full text-left border-collapse min-w-[560px]">
              ...
              <tbody className="divide-y divide-slate-800">
                {creativeStats.map((c: any, idx: number) => (
                  ...
                  >
                    <td className="p-4">
                      <div className="font-bold text-slate-200 line-clamp-1 max-w-xs group-hover:text-indigo-400 transition-colors">
                        {c.headline}
                      </div>
                      <div className="text-xs text-slate-500 flex items-center gap-2 mt-0.5">
                        <span className="uppercase">{c.platform}</span>
                        {c.is_best_creative && (
                          <span className="text-emerald-500 font-bold px-1.5 py-0.5 bg-emerald-500/10 rounded">
                            BEST PICK
                          </span>
                        )}
                      </div>
          ...
          <div className="p-12 text-center text-slate-500 flex flex-col items-center">
            ...
            <p className="text-sm mt-1">
              Run a campaign to generate creatives and view their projected
              impact here.
            </p>
          </div>
        )}
