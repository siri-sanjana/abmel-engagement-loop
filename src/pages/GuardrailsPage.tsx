...
export const GuardrailsPage = () => {
  ...
  const [files, setFiles] = useState<GuardrailFile[]>(
    input.brandGuidelines
      ? [
          {
            id: "bg-1",
            name: input.brandGuidelines || "Brand Guidelines",
            size: "—",
            type: "Uploaded",
            uploadedAt: "Current session",
          },
        ]
      : [],
  );
  ...
  const rules = useMemo(() => {
    const base = [
      {
        id: 1,
        name: "Brand Tone Verification",
        level: "Strict",
        type: "NLP Analysis",
      },
      {
        id: 2,
        name: "Regulatory Compliance (GDPR)",
        level: "Critical",
        type: "Policy Check",
      },
    ];
      ...
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          ...
          <p className="text-slate-400">
            Automated safety and compliance protocols governing agent outputs.
          </p>
        </div>
      </div>
        ...
        <div className="p-6 border-b border-slate-800 flex items-center justify-between">
          <div>
            <h3 className="font-bold text-white text-lg flex items-center gap-2">
              <Upload size={18} className="text-indigo-400" /> Brand Guardrail
              Documents
            </h3>
            <p className="text-xs text-slate-500 mt-1">
              Upload PDF, DOCX, or TXT files. Files are saved to the database
              and linked to your campaign.
            </p>
          </div>
          <button
            ...
            {files.length === 0 ? (
              <div className="p-10 text-center text-slate-500">
                ...
                <p className="font-medium text-slate-400">
                  No guardrail documents uploaded
                </p>
                <p className="text-sm mt-1">
                  Upload a PDF, DOCX, or TXT file to apply custom brand
                  constraints.
                </p>
              </div>
            ) : (
        ...
        <div className="divide-y divide-slate-800">
          {rules.map((rule, idx) => (
            ...
            >
              ...
              <div className="flex items-center gap-4">
                <span className="text-xs font-bold text-orange-400 bg-orange-500/10 px-3 py-1 rounded-full uppercase tracking-wide">
                  {rule.level}
                </span>
        ...
        <div className="p-4 bg-yellow-500/10 border border-yellow-500/20 rounded-xl flex items-center gap-4 text-yellow-400">
          ...
          <div>
            ...
            <p className="text-sm opacity-80">
              Default global safety rules are active, but custom brand voice
